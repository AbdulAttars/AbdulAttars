...
export const WELCOME_PROMO_CODE = "ABDULS5";
export const WELCOME_DISCOUNT_CENTS = 500;
export const WELCOME_MINIMUM_CENTS = 3000;
...
export function normalizeCheckoutRequest(body) {
  ...
  const promoCode = safeText(body.promo_code, 40).toUpperCase().replace(/\s+/g, "");
  if (promoCode && promoCode !== WELCOME_PROMO_CODE) {
    throw new Error("That promotion code is not valid.");
  }
  if (promoCode && subtotalCents < WELCOME_MINIMUM_CENTS) {
    throw new Error("ABDULS5 requires a bottle subtotal of at least $30.00.");
  }
  if (promoCode && !isEmail(normalizedCustomer.contact)) {
    throw new Error("ABDULS5 requires an email address in the contact field.");
  }

  if (isEmail(normalizedCustomer.contact)) {
    normalizedCustomer.contact = normalizedCustomer.contact.toLowerCase();
  }

  const discountCents = promoCode ? WELCOME_DISCOUNT_CENTS : 0;
  const taxableSubtotalCents = Math.max(0, subtotalCents - discountCents);
  const taxCents = Math.round(taxableSubtotalCents * SALES_TAX_RATE);
  ...
  return {
    ...
    taxCents,
    deliveryCents,
    totalCents: taxableSubtotalCents + taxCents + deliveryCents
  };
}
...
export async function assertWelcomePromotionEligible(contact) {
  const email = safeText(contact, 254).toLowerCase();
  if (!isEmail(email)) throw new Error("ABDULS5 requires a valid email address.");

  const response = await fetch(
    `${process.env.SUPABASE_URL.replace(/\/$/, "")}/rest/v1/orders` +
      `?select=payment_status,status&contact=ilike.${encodeURIComponent(email)}&limit=100`,
    { headers: supabaseServerHeaders() }
  );
  if (!response.ok) throw new Error("Promotion eligibility could not be verified. Please try again.");

  const rows = await response.json();
  const hasCompletedOrder = (Array.isArray(rows) ? rows : []).some((row) => {
    const payment = safeText(row && row.payment_status, 100).toLowerCase();
    const status = safeText(row && row.status, 100).toLowerCase();
    return payment.startsWith("paid") || /^(paid|completed|delivered|fulfilled)/.test(status);
  });
  if (hasCompletedOrder) {
    throw new Error("ABDULS5 is available only on your first purchase.");
  }
}

export async function savePendingOrder(orderCode, checkout) {
  const { customer, items, promoCode, subtotalCents, discountCents, taxCents, totalCents, deliveryCents } = checkout;
  const notes = [
    ...
    promoCode ? `Promotion: ${promoCode} (-$${(discountCents / 100).toFixed(2)})` : "",
